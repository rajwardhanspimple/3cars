...
A desktop-browser racing game built with Babylon.js. This branch is an approved mountain preview only. Race two AI drivers over five laps of a Sakura Pass preview route with mountain scenery and three full-detail Mustang racers.
...
If you skip a checkpoint, the current lap stays invalid and that lap cannot become a record. Keep driving and the next lap works normally. Manual recovery on `R` still adds 20 seconds. Existing cut penalties of 5 seconds remain active.
...
Manual checks still required: visual approval of the mountain preview branch, drive a full race, confirm invalid-lap HUD persistence after a skipped checkpoint, inspect Sakura Pass labels, compare Dry dusk and Wet dusk, check pause/restart, and confirm smooth camera movement on your GPU.
...
Serve `dist/` over HTTP. Paths are relative. Main is not published from this branch. This preview awaits visual approval before any main release decision.
