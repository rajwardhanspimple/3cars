...
function resetRace(start=false){
 ...
 if(start)race.start();previousPhase=null;syncPhase();updateHUD();if(start)ui['race-canvas'].focus();else ui.start.focus();
}
function syncPhase(){
 ...
 if(race.phase==='paused')$('resume').focus();
...
function finishRace(){
 ...
 ui['result-list'].replaceChildren();
 for(const [i,c] of order.entries()){
  const li=document.createElement('li'),name=document.createElement('span'),right=document.createElement('span'),detail=document.createElement('small');
  name.textContent=`${i+1}. ${c.name}`;right.textContent=formatTime(c.finishTime+c.penalty);detail.textContent=`${c.model.name} / penalties +${c.penalty}s`;name.append(detail);li.append(name,right);ui['result-list'].append(li);
 }
...
function updateHUD(){
 ...
 const note=p.finished&&race.phase==='racing'?'Finished. Waiting for the other drivers.':p.noticeUntil>race.time?p.notification:'';text('notification',note);
 ui.countdown.hidden=race.phase!=='countdown';if(race.phase==='countdown')text('countdown',Math.max(1,Math.ceil(race.countdown)));
 ui.standings.replaceChildren();
 for(const [i,c] of order.entries()){
  const li=document.createElement('li');if(c.index===0)li.className='you';
  const pos=document.createElement('span'),name=document.createElement('span'),gap=document.createElement('span');pos.textContent=i+1;name.textContent=c.name;
  if(c.finished)gap.textContent=formatTime(c.finishTime+c.penalty);else if(i===0)gap.textContent='Leader';else gap.textContent=`~+${((order[0].progress-c.progress)/Math.max(10,c.speed)).toFixed(1)}s`;
  li.append(pos,name,gap);ui.standings.append(li);
 }
 drawMap();
}
...
function drawMap(){
 ...
 const start=project(t.at(0));map.fillStyle='#173c56';map.fillRect(start[0]-3,start[1]-4,6,8);
 for(const c of [...race.cars].reverse()){const [x,y]=project(c);map.beginPath();map.arc(x,y,c.index===0?5:4,0,Math.PI*2);map.fillStyle=c.model.color;map.fill();map.strokeStyle=c.index===0?'#173c56':'#fff';map.lineWidth=2;map.stroke();}
 map.fillStyle='#173c56';map.font='11px Segoe UI, Arial';map.fillText('Meridian circuit',12,182);
}
function togglePause(){keys.clear();race.pause();syncPhase();if(race.phase!=='paused')ui['race-canvas'].focus();}
...
function autoPause(){keys.clear();if(['countdown','racing'].includes(race.phase)){race.pause();syncPhase();}audio.update({phase:'paused'});}
...
document.addEventListener('keydown',event=>{
 if(event.key!=='Tab')return;const modal=[ui['pause-panel'],ui.results,ui.error].find(n=>!n.hidden);if(!modal)return;
...
function frame(now){
 try{
  ...
  if(!document.hidden){
   accumulator+=elapsed;const input={throttle:pressed('KeyW','ArrowUp'),brake:pressed('KeyS','ArrowDown','Space'),steer:pressed('KeyD','ArrowRight')-pressed('KeyA','ArrowLeft')};
   while(accumulator>=1/120){race.step(1/120,input);accumulator-=1/120;}
   syncPhase();const p=race.player;audio.update({phase:race.phase,rpm:p.rpm,speed:p.speed,throttle:p.throttle,wet:race.weather==='wet',slip:p.slip,impact:p.impact,gear:p.gear},elapsed);
   hudClock+=elapsed;if(hudClock>.1){updateHUD();hudClock=0;}
   view.render(race.phase==='paused'?0:elapsed);
  }
