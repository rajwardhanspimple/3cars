...
const records=new CityRecords(),audio=new RaceAudio(),keys=new Set();
let settings=records.loadSettings();installCitySetup(settings);
const ids=['menu','hud','pause-panel','results','error','error-message','setup','start','track','mode','density','off-course','weather','quality','mute','saved-best','history-count','race-history','storage-status','race-canvas','position','lap','race-time','standings','current-lap','best-lap','penalty','engine-condition','steering-condition','tires-condition','rpm','rpm-fill','gear','speed','abs','tc','conditions','notification','countdown','minimap','result-title','result-summary','result-list','nitro-meter','nitro-value','nitro-status','drift-status','track-label','circuit-caption','game-title','track-intro','history-label'];
const ui=Object.fromEntries(ids.map(id=>[id,$(id)]));
let race=new Race(settings),view,simulation,saved=false,loading=true,viewReady=false,loadRevision=0,disposed=false,failed=false,pausePending=false,windowFocused=document.hasFocus(),loadLostFocus=false,last=performance.now(),previousPhase=null,frameId,lastSound=performance.now(),lastHUD=0;
race.route=trackInfo(settings.trackId);records.selectTrack(settings.trackId,settings.mode);
...
const selected=()=>({carId:ui.setup.elements.carId.value,trackId:ui.track.value,mode:ui.mode.value,density:ui.density.value,weather:ui.weather.value,quality:ui.quality.value,muted:settings.muted});
...
function storageStatus(){text('storage-status',records.available?`Settings, best laps, and the last 20 ${trackInfo(settings.trackId).name} races stay in this browser. Track and mode records are separate.`:'Browser storage is unavailable. This session will not be saved.');}
function applyTrackCopy(){
 const track=trackInfo(settings.trackId),city=track.id==='anime-night-city-v1';
 ui.mode.disabled=!city;ui.density.disabled=!city;
 document.title=`3cars | ${track.name}`;document.body.dataset.track=track.id;
 text('track-label',`${track.name} / ${track.label}`);text('circuit-caption',track.name);text('game-title',track.title);text('track-intro',track.intro);text('history-label',`Recent ${track.name} ${city&&settings.mode==='free-roam'?'free-roam':'race'} results`);
...
function persistSettings(){settings=selected();records.saveSettings(settings);records.selectTrack(settings.trackId,settings.mode);applyTrackCopy();storageStatus();}
function setLoading(value,label='Loading car...'){loading=value;ui.setup.inert=value;ui.start.disabled=value;text('start',value?label:race.freeRoam?'Drive and explore':'Start five-lap race');document.body.dataset.loading=String(value);if(value)clearInput();}
function renderHistory(){
 records.selectTrack(settings.trackId,settings.mode);
 ...
 if(!rows.length){const li=document.createElement('li');li.textContent=`Finish your first ${trackInfo(settings.trackId).name} session to record a result.`;ui['race-history'].append(li);}
...
function applySnapshot(state){
 ...
 if(Array.isArray(state.traffic))race.traffic=state.traffic;
 if(Array.isArray(state.pedestrians))race.pedestrians=state.pedestrians;
 for(let i=0;i<race.cars.length;i++)Object.assign(race.cars[i],state.cars[i]);
 race.player=race.cars[0];
 if(view?.scene)view.scene.metadata={...view.scene.metadata,simulation:{phase:race.phase,time:race.time,steer:race.player.steer,nitro:race.player.nitro,boostActive:race.player.boostActive,drifting:race.player.drifting,props:race.props,routeId:race.route.id,routeName:race.route.name,mode:race.mode}};
...
async function resetRace(start=false,focusTarget=null){
 ...
 try{
  const physicsReady=simulation.reset(config);physicsReady.catch(()=>{});
  ...
  if(race.cityEnabled){
   if(!view.districtCollisionLayout)throw Error('City collision layout is missing');
   await simulation.command('district',{colliders:view.districtCollisionLayout});
  }
...
function finishRace(){
 ...
 if(!saved){saved=true;records.selectTrack(race.route.id,race.mode);records.recordRace({carId:p.model.id,weather:race.weather,position,time:p.finishTime,penalty:p.penalty,bestLap:p.bestLap,finishedAt:new Date().toISOString()});renderHistory();}
}
function updateHUD(){
 ...
 const pausedLap=!!(p.offCourse||p.rejoinPending);ui['off-course'].hidden=!pausedLap;
 text('off-course',p.offCourse?'Off circuit. Lap and checkpoints paused.':`Lap paused. Return before checkpoint ${(p.nextGate%race.track.gates)+1} to resume.`);
