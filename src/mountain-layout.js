// One deterministic layout for scenery and collision rules. The preview retains
// the loop while replacing its road treatment and adding actual road elevation.
...
export function mountainLayout(track){
 ...
 // Alternating mature trees extend canopies over shoulders. Trunks remain solid
 // and outside the 18m road. All instances share the downloaded source meshes.
 ...
 for(let s=55,n=0;s<track.length;s+=85,n++){
  const p=track.at(s,n%2?11.3:-11.3);props.push({id:`cone-${n}`,type:n%4===0?'sign':'cone',x:p.x,z:p.z,y:roadHeight(p.x,p.z),yaw:p.heading,radius:.45,mass:5});
 }
 // Short retaining walls follow the inner side of the tightest turn sectors.
