...
function sourceBaseCenter(meshes, min, max) {
 ...
 const src = points.length ? points : [v((min.x + max.x) / 2, min.y, (min.z + max.z) / 2)];
 let sx = 0, sz = 0; for (const p of src) { sx += p.x; sz += p.z; }
...
async function addAssetTrees(view, layout, counts, shadowRoots) {
 ...
 try {
  ...
  const green = await loadTreeSource(view, 'green', '../assets/environment/green-tree/scene.gltf', 13.5); sources.push(green);
