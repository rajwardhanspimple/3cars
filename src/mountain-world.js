...
function loadTexture(scene, url, name, uScale = 1, vScale = uScale) {
 return new Promise((resolve, reject) => { const tex = new B.Texture(url, scene, false, false, B.Texture.TRILINEAR_SAMPLINGMODE, () => resolve(tex), () => reject(new Error(`Texture failed to load: ${name}`))); tex.name = name; tex.uScale = uScale; tex.vScale = vScale; });
}
function assignMetallicRoughness(mat, tex) { mat.metallicTexture = tex; mat.useRoughnessFromMetallicTextureAlpha = false; mat.useRoughnessFromMetallicTextureGreen = true; mat.useMetallnessFromMetallicTextureBlue = false; mat.useMetalnessFromMetallicTextureBlue = false; mat.metallic = 0; }
function slopePose(node, x, z, yaw = 0, yOffset = 0, tilt = 0) { const pose = roadPose(x, z, yaw); node.position.set(x, pose.y + yOffset, z); node.rotation.set(pose.pitch, yaw, pose.roll + tilt); return pose; }
function flipIfDown(name, positions, indices) {
 const normals = []; B.VertexData.ComputeNormals(positions, indices, normals); let y = 0;
 for (let i = 1; i < normals.length; i += 3) y += normals[i];
 if (y < 0) { for (let i = 0; i < indices.length; i += 3) { const t = indices[i + 1]; indices[i + 1] = indices[i + 2]; indices[i + 2] = t; } normals.length = 0; B.VertexData.ComputeNormals(positions, indices, normals); let check = 0; for (let i = 1; i < normals.length; i += 3) check += normals[i]; if (check < 0) throw new Error(`${name} normals point downward after flip`); }
 return normals;
}
function stripMesh(scene, name, left, right, material, repeat = 8) {
 const positions = [], indices = [], uvs = [];
 for (let i = 0; i < left.length; i++) { const a = left[i], b = right[i], width = Math.hypot(a.x - b.x, a.z - b.z), s = a.s ?? i; positions.push(a.x, a.y, a.z, b.x, b.y, b.z); uvs.push(s / repeat, 0, s / repeat, width / repeat); if (i < left.length - 1) { const n = i * 2; indices.push(n, n + 2, n + 1, n + 1, n + 2, n + 3); } }
 const mesh = new B.Mesh(name, scene), data = new B.VertexData(); data.positions = positions; data.indices = indices; data.uvs = uvs; data.normals = flipIfDown(name, positions, indices); data.applyToMesh(mesh); if (mesh.getTotalVertices() * 2 !== uvs.length) throw new Error(`${name} UV length mismatch`); mesh.material = material; mesh.receiveShadows = true; return mesh;
}
function roadRibbon(view, name, inner, outer, material, y = .035, repeat = 8) {
 const t = view.race.track, left = [], right = [];
 for (let i = 0; i <= t.points.length; i++) { const s = i === t.points.length ? t.length : t.points[i].s, a = t.at(s, inner), b = t.at(s, outer); left.push({ x: a.x, y: roadHeight(a.x, a.z) + y, z: a.z, s }); right.push({ x: b.x, y: roadHeight(b.x, b.z) + y, z: b.z, s }); }
...
function addRoad(view, materials, counts) {
 ...
 roadRibbon(view, 'left-white-edge-paint', -HALF_WIDTH + .16, -HALF_WIDTH + .38, materials.white, .065, 1.5); roadRibbon(view, 'right-white-edge-paint', HALF_WIDTH - .38, HALF_WIDTH - .16, materials.white, .065, 1.5);
 roadRibbon(view, 'double-yellow-center-a', -.22, -.06, materials.yellow, .075, 1.4); roadRibbon(view, 'double-yellow-center-b', .06, .22, materials.yellow, .075, 1.4);
 roadRibbon(view, 'banked-left-curb', -HALF_WIDTH - .92, -HALF_WIDTH - .18, materials.curb, .08, 2.2); roadRibbon(view, 'banked-right-curb', HALF_WIDTH + .18, HALF_WIDTH + .92, materials.curb, .08, 2.2);
 counts.roadWidth = HALF_WIDTH * 2; counts.roadUsesRoadHeight = true;
}
function terrainHeight(track, x, z, bounds, cellDiag = 20) {
 const p = track.project(x, z), roadY = roadHeight(x, z), corridor = HALF_WIDTH + cellDiag + 3;
 if (p.distance <= corridor) return roadY - .16;
 const hills = 9 + .038 * (z - bounds.centerZ) + Math.sin(x * .013) * 8 + Math.cos(z * .011) * 11 + Math.sin((x - z) * .006) * 7, slope = Math.max(0, p.distance - 55) * .09, cut = smoothstep(corridor, BARRIER + 46, p.distance), shoulder = roadY - .16 - Math.max(0, p.distance - HALF_WIDTH) * .035;
 return shoulder * (1 - cut) + (hills + slope) * cut;
}
function barycentricHeight(p, a, b, c) { const v0x = b.x - a.x, v0z = b.z - a.z, v1x = c.x - a.x, v1z = c.z - a.z, v2x = p.x - a.x, v2z = p.z - a.z, d00 = v0x*v0x + v0z*v0z, d01 = v0x*v1x + v0z*v1z, d11 = v1x*v1x + v1z*v1z, d20 = v2x*v0x + v2z*v0z, d21 = v2x*v1x + v2z*v1z, denom = d00*d11 - d01*d01 || 1, w1 = (d11*d20 - d01*d21)/denom, w2 = (d00*d21 - d01*d20)/denom, w0 = 1 - w1 - w2; return a.y*w0 + b.y*w1 + c.y*w2; }
function addTerrain(view, materials, bounds, counts) {
 const scene = view.scene, track = view.race.track, size = 1040, cells = view.quality === 'high' ? 74 : 52, half = size / 2, step = size / cells, cellDiag = Math.SQRT2 * step, ox = bounds.centerX - half, oz = bounds.centerZ - half, positions = [], indices = [], uvs = [];
 for (let iz = 0; iz <= cells; iz++) for (let ix = 0; ix <= cells; ix++) { const x = ox + ix * step, z = oz + iz * step, y = terrainHeight(track, x, z, bounds, cellDiag); positions.push(x, y, z); uvs.push(x / 7, z / 7); }
 for (let iz = 0; iz < cells; iz++) for (let ix = 0; ix < cells; ix++) { const a = iz * (cells + 1) + ix, b = a + 1, c = a + cells + 1, d = c + 1; indices.push(a, c, b, b, c, d); }
 const mesh = new B.Mesh('smooth-mountain-roadcut-terrain', scene), data = new B.VertexData(); data.positions = positions; data.indices = indices; data.uvs = uvs; data.normals = flipIfDown('smooth-mountain-roadcut-terrain', positions, indices); data.applyToMesh(mesh); mesh.material = materials.ground; mesh.receiveShadows = true;
 const vertex = (ix, iz) => { const i = (iz * (cells + 1) + ix) * 3; return { x: positions[i], y: positions[i + 1], z: positions[i + 2] }; };
 view.sceneryGroundHeight = (x, z) => { const fx = clamp((x - ox) / step, 0, cells - .0001), fz = clamp((z - oz) / step, 0, cells - .0001), ix = Math.floor(fx), iz = Math.floor(fz), tx = fx - ix, tz = fz - iz, a = vertex(ix, iz), b = vertex(ix + 1, iz), c = vertex(ix, iz + 1), d = vertex(ix + 1, iz + 1), p = { x, z }; return tx + tz <= 1 ? barycentricHeight(p, a, c, b) : barycentricHeight(p, b, c, d); };
 counts.terrainVertices = positions.length / 3; counts.terrainRoadCutUsesRoadHeight = true; counts.terrainFlatMargin = Number((HALF_WIDTH + cellDiag + 3).toFixed(2)); counts.groundHeightSampler = 'triangle-barycentric-terrain-grid'; return mesh;
}
function localPoint(root, lx, ly, lz) { return B.Vector3.TransformCoordinates(v(lx, ly, lz), root.getWorldMatrix()); }
function makeWall(view, wall, material) {
 ...
 const w = wall.width, h = wall.height, l = wall.length, y0 = -.04, y1 = h - .04, corners = [[-w/2,y0,-l/2],[w/2,y0,-l/2],[w/2,y1,-l/2],[-w/2,y1,-l/2],[-w/2,y0,l/2],[w/2,y0,l/2],[w/2,y1,l/2],[-w/2,y1,l/2]]; root.computeWorldMatrix(true);
 ...
 root.dispose(); const mesh = new B.Mesh('moss-stone-retaining-wall', view.scene), data = new B.VertexData(); data.positions = positions; data.indices = indices; data.uvs = uvs; data.normals = flipIfDown('moss-stone-retaining-wall', positions, indices); data.applyToMesh(mesh); mesh.material = material; mesh.receiveShadows = true; return mesh;
}
function addWalls(view, layout, materials, counts) { const meshes = layout.walls.map(wall => makeWall(view, wall, materials.rock)); view.merge(meshes, 'batched-moss-stone-retaining-walls'); counts.retainingWalls = layout.walls.length; }
function addGuardrails(view, materials, counts) {
 const track = view.race.track, rails = [], posts = [];
 for (let s = 0; s < track.length; s += 7.5) for (const side of [-1, 1]) { const p = track.at(s, side * 16.95), yaw = p.heading;
  ...
  const rail = B.MeshBuilder.CreateBox('barrier-visible-guardrail', { width: .22, height: .34, depth: 7.7 }, view.scene); slopePose(rail, p.x, p.z, yaw, 1.15); rail.material = materials.rail; rail.receiveShadows = true; rails.push(rail);
 }
 view.merge(posts, 'batched-visible-barrier-posts'); view.merge(rails, 'batched-visible-physics-guardrails'); counts.visibleBarrierRails = rails.length; counts.visibleBarrierPosts = posts.length; counts.physicsBarrierOffset = 16.95; counts.guardrailSegmentLength = 7.7;
}
function makeProp(view, prop, materials) {
 ...
 if (prop.type === 'sign') { view.box('mountain-sign-post', [.18, 2.2, .18], [0, 1.1, 0], materials.dark, root); view.box('mountain-sign-panel', [2.5, 1.05, .12], [0, 2.35, 0], materials.white, root); view.sign('HAIRPIN', [0, 2.35, -.08], 2.2, .82, root); }
 else { const cone = B.MeshBuilder.CreateCylinder('shared-layout-cone', { height: 1.15, diameterTop: .08, diameterBottom: .62, tessellation: 18 }, view.scene); cone.position.y = .58; cone.parent = root; cone.material = materials.orange; cone.receiveShadows = true; }
 return root;
}
function addProps(view, layout, materials, counts) { const propNodes = new Map(); for (const prop of layout.props) propNodes.set(prop.id, makeProp(view, prop, materials)); counts.sharedLayoutProps = layout.props.length; return propNodes; }
function makeRoof(scene, name, width, depth, rise, y, mat, parent) { const hw = width/2, hd = depth/2, positions = [-hw,y,-hd, hw,y,-hd, hw,y,hd, -hw,y,hd, 0,y+rise,-hd*.86, 0,y+rise,hd*.86], indices = [0,1,4,3,5,2,0,4,5,0,5,3,1,2,5,1,5,4], uvs = [0,0,1,0,1,1,0,1,.5,0,.5,1], mesh = new B.Mesh(name, scene), data = new B.VertexData(); data.positions = positions; data.indices = indices; data.uvs = uvs; data.normals = flipIfDown(name, positions, indices); data.applyToMesh(mesh); mesh.material = mat; mesh.parent = parent; mesh.receiveShadows = true; return mesh; }
function addMountainsAndTemple(view, materials, bounds, counts) {
 const scene = view.scene, meshes = [];
 for (let i = 0; i < 18; i++) { const a = i / 18 * Math.PI * 2, m = B.MeshBuilder.CreateCylinder('dusk-layered-mountain-backdrop', { height: 120 + (i % 5) * 34, diameterTop: 10, diameterBottom: 230 + (i % 4) * 45, tessellation: 8 }, scene); m.position = v(bounds.centerX + Math.sin(a) * 740, roadHeight(bounds.centerX, bounds.centerZ) - 20, bounds.centerZ + Math.cos(a) * 740); m.rotation.y = a; m.material = i % 2 ? materials.mountainNear : materials.mountainFar; meshes.push(m); }
 view.merge(meshes, 'batched-dusk-mountain-backdrop'); const p = view.race.track.at(view.race.track.length * .68, -118), baseY = (view.sceneryGroundHeight ? view.sceneryGroundHeight(p.x, p.z) : roadHeight(p.x, p.z) - .16), temple = new B.TransformNode('distant-ridge-tiered-temple', scene); temple.position = v(p.x, baseY, p.z); temple.rotation.y = p.heading + Math.PI;
 view.box('temple-stone-foundation', [24, 2.2, 17], [0, 1.1, 0], materials.rock, temple); view.box('temple-lower-hall', [17, 5.2, 11], [0, 4.8, 0], materials.templeWall, temple); makeRoof(scene, 'temple-lower-sloped-roof', 25, 18, 2.2, 7.7, materials.templeRoof, temple); view.box('temple-upper-hall', [11, 4.1, 7.5], [0, 10.4, 0], materials.templeWall, temple); makeRoof(scene, 'temple-upper-sloped-roof', 18, 13, 1.8, 12.6, materials.templeRoof, temple); view.box('temple-lantern-tower', [3.2, 5.4, 3.2], [0, 16.0, 0], materials.templeWall, temple); makeRoof(scene, 'temple-cap-sloped-roof', 7, 6, 1.3, 18.7, materials.templeRoof, temple); counts.mountainBackdrop = 18; counts.templeBackdrop = 1;
}
function makeSky(view, materials) {
 ...
 ctx.globalAlpha = .28; for (let i = 0; i < 32; i++) { const x = (i * 173) % 1140 - 80, y = 55 + (i * 61) % 210, w = 95 + (i % 6) * 28, grad = ctx.createRadialGradient(x, y, 1, x, y, w); grad.addColorStop(0, 'rgba(255,219,174,.75)'); grad.addColorStop(1, 'rgba(255,219,174,0)'); ctx.fillStyle = grad; ctx.beginPath(); ctx.ellipse(x, y, w, w * .23, 0, 0, Math.PI * 2); ctx.fill(); }
 ctx.globalAlpha = 1; tex.update(); const mat = new B.StandardMaterial('unlit-warm-dusk-sky-material', scene); mat.diffuseTexture = tex; mat.emissiveColor = color('#ffffff'); mat.disableLighting = true; mat.backFaceCulling = false; const sky = B.MeshBuilder.CreateBox('warm-dusk-cloud-skybox', { size: 1500 }, scene); sky.material = mat; sky.infiniteDistance = true; sky.isPickable = false; materials.sky = mat;
}
...
function addPetals(view, materials, counts, petalSources) {
 ...
 const pickSource = player => { if (!petalSources.length) return null; const near = player ? petalSources.filter(s => { const dx = s.x - player.x, dz = s.z - player.z; return dx * dx + dz * dz <= 70 * 70; }) : []; const list = near.length ? near : petalSources; return list[(rnd() * list.length) | 0]; };
 const spawn = (p, player) => {
  ...
  p.sourceId = source.id; p.spawnX = x; p.spawnY = y; p.spawnZ = z; p.canopyRadius = source.radius; p.sourceCenter = { x: source.x, z: source.z }; p.x = x; p.y = y; p.z = z; p.vx = .18 + (rnd() - .5) * .28; p.vy = -.16 - rnd() * .18; p.vz = .08 + (rnd() - .5) * .24; p.age = 0; p.life = 6 + rnd() * 5; p.spin = (rnd() - .5) * 5; p.phase = rnd() * Math.PI * 2; p.swirl = 0; p.mesh.isVisible = true; p.mesh.position.set(x, Math.max(y, groundAt(x, z) + .18), z);
 };
 const seedInitial = player => { for (const p of petals) if (!p.sourceId) spawn(p, player); };
 for (let i = 0; i < max; i++) { const mesh = base.createInstance(`air-petal-${i}`); mesh.isVisible = false; mesh.isPickable = false; petals.push({ mesh, seed: i * 97.13, age: 99, sourceId: null, spawnX: null, spawnY: null, spawnZ: null, canopyRadius: null, x: 0, y: 0, z: 0 }); }
 ...
 return { petals, spawn, seedInitial, groundAt, rnd };
}
function updatePetals(view, petalSystem, dt, player, phase) {
 const { petals, spawn, groundAt } = petalSystem; if (phase === 'paused' || dt <= 0) return;
 const cars = view.race?.cars || [], windX = .38, windZ = .14;
 for (const p of petals) {
  if (!p.sourceId) spawn(p, player);
  if (!p.sourceId) continue;
  p.age += dt; p.vx += windX * dt * .08 + Math.sin(p.age * 2.1 + p.seed) * dt * .12; p.vz += windZ * dt * .08 + Math.cos(p.age * 1.7 + p.seed) * dt * .10; p.vy -= .42 * dt;
  for (const car of cars) { const dx = p.x - car.x, dz = p.z - car.z, d2 = dx * dx + dz * dz; if (d2 < 9) { const strength = (1 - Math.sqrt(d2) / 3) * dt * 1.6, cvx = Number.isFinite(car.vx) ? car.vx : Math.sin(car.yaw || 0) * (car.speed || 0), cvz = Number.isFinite(car.vz) ? car.vz : Math.cos(car.yaw || 0) * (car.speed || 0); p.vx += cvx * strength * .28 - dz * strength * .7; p.vz += cvz * strength * .28 + dx * strength * .7; p.vy += strength * 1.2; p.swirl = Math.min(1, p.swirl + strength * 3); } }
  p.swirl = Math.max(0, p.swirl - dt * 1.7); p.x += p.vx * dt + Math.sin(p.age * 4.5 + p.seed) * (.08 + p.swirl * .22); p.z += p.vz * dt + Math.cos(p.age * 3.7 + p.seed) * (.06 + p.swirl * .18); p.y += p.vy * dt + Math.sin(p.age * 5.2 + p.seed) * .025;
  const ground = groundAt(p.x, p.z), sd = p.sourceCenter ? Math.hypot(p.x - p.sourceCenter.x, p.z - p.sourceCenter.z) : 0;
  if (p.y <= ground + .08 || p.age > p.life || sd > 95) { spawn(p, player); continue; }
  p.mesh.position.set(p.x, Math.max(p.y, ground + .1), p.z); p.mesh.rotation.set(p.age * 1.9 + p.seed, Math.sin(p.age * 3 + p.seed) * .7, p.age * p.spin + p.swirl * 4);
 }
}
function petalState(petalSystem) { return petalSystem.petals.filter(p => p.sourceId).map(p => ({ sourceId: p.sourceId, spawnX: p.spawnX, spawnY: p.spawnY, spawnZ: p.spawnZ, canopyRadius: p.canopyRadius, x: p.x, y: p.y, z: p.z })); }
