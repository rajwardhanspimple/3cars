...
export class RaceView {
 ...
 setWeather(weather){const wet=weather==='wet';this.scene.clearColor=wet?new B.Color4(.34,.42,.48,1):new B.Color4(.72,.82,.88,1);this.scene.fogColor=wet?color('#71838e'):color('#bed0d7');this.scene.fogDensity=wet?.0038:(this.quality==='high'?.00145:.0019);this.sun.intensity=wet?.62:2.25;this.hemi.intensity=wet?.78:.92;if(this.roadMaterial){this.roadMaterial.roughness=wet?.19:.86;this.roadMaterial.metallic=wet?.24:0;this.roadMaterial.environmentIntensity=wet?1.05:.82;}if(this.sceneryMaterials?.seal){this.sceneryMaterials.seal.roughness=wet?.16:.38;this.sceneryMaterials.seal.metallic=wet?.2:0;}if(this.rain)this.rain.setEnabled(wet);}
