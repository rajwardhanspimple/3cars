...
window.addEventListener('resize',this.resize);
 ...
 band(name,inner,outer,material,y=.025){const t=this.race.track,positions=[],indices=[],uvs=[],width=Math.abs(outer-inner);for(let i=0;i<=t.points.length;i++){const s=i===t.points.length?t.length:t.points[i].s,a=t.at(s,inner),b=t.at(s,outer);positions.push(a.x,y,a.z,b.x,y,b.z);uvs.push(s/8,0,s/8,width/8);if(i<t.points.length){const n=i*2;indices.push(n,n+2,n+1,n+1,n+2,n+3);}}const m=new B.Mesh(name,this.scene),data=new B.VertexData();data.positions=positions;data.indices=indices;data.uvs=uvs;const normals=[];B.VertexData.ComputeNormals(positions,indices,normals);data.normals=normals;data.applyToMesh(m);if(m.getTotalVertices()*2!==uvs.length)throw new Error(`${name} UV length mismatch`);m.material=material;m.receiveShadows=true;return m;}
