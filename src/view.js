...
export class RaceView {
 constructor(canvas,race,{quality='high'}={}) {
  ...
  scene.imageProcessingConfiguration.toneMappingEnabled=true;scene.imageProcessingConfiguration.toneMappingType=B.ImageProcessingConfiguration.TONEMAPPING_ACES;scene.imageProcessingConfiguration.exposure=1.12;scene.imageProcessingConfiguration.contrast=1.12;
  ...
  this.shadow=new B.ShadowGenerator(quality==='high'?2048:1024,this.sun);this.shadow.usePercentageCloserFiltering=true;this.shadow.filteringQuality=quality==='high'?B.ShadowGenerator.QUALITY_MEDIUM:B.ShadowGenerator.QUALITY_LOW;this.shadow.bias=.001;this.shadow.normalBias=.025;this.shadow.darkness=.24;
...
box(name,size,pos,material,parent=null){const mesh=B.MeshBuilder.CreateBox(name,{width:size[0],height:size[1],depth:size[2]},this.scene);mesh.position=v(...pos);mesh.material=material;if(parent)mesh.parent=parent;mesh.receiveShadows=true;return mesh;}
 texture(name,base,noise=12){const t=new B.DynamicTexture(name,{width:512,height:512},this.scene,false),ctx=t.getContext(),img=ctx.createImageData(512,512);let seed=713;for(let i=0;i<img.data.length;i+=4){seed=(seed*1664525+1013904223)>>>0;const n=(seed/4294967296-.5)*noise;for(let c=0;c<3;c++)img.data[i+c]=base[c]+n;img.data[i+3]=255;}ctx.putImageData(img,0,0);t.update();t.uScale=26;t.vScale=26;return t;}
 band(name,inner,outer,material,y=.025){const paths=[[],[]],t=this.race.track;for(let i=0;i<=t.points.length;i++){const s=i===t.points.length?t.length:t.points[i].s;for(const [j,offset]of[inner,outer].entries()){const p=t.at(s,offset);paths[j].push(v(p.x,y,p.z));}}const m=B.MeshBuilder.CreateRibbon(name,{pathArray:paths,sideOrientation:B.Mesh.DOUBLESIDE},this.scene);m.material=material;m.receiveShadows=true;return m;}
 ...
 freezeStaticWorld(){for(const mesh of this.scene.meshes)mesh.freezeWorldMatrix();}
...
createWorld(){
  ...
  this.band('runoff',-BARRIER,BARRIER,concrete,0);this.band('circuit',-HALF_WIDTH,HALF_WIDTH,asphalt,.025);this.band('left-line',-HALF_WIDTH+.16,-HALF_WIDTH+.32,white,.035);this.band('right-line',HALF_WIDTH-.32,HALF_WIDTH-.16,white,.035);
  ...
  for(let s=0,n=0;s<t.length;s+=3.5,n++)for(const side of[-1,1]){
   ...
   if(n%2===0){const b=t.at(s,side*(BARRIER+.45)),wall=this.box('safety-wall',[.7,1.1,7.1],[b.x,.5,b.z],n%8<4?white:blue);wall.rotation.y=b.heading;walls.push(wall);}
   if(n%5===0){const b=t.at(s,side*(BARRIER+1.1));fences.push(this.box('fence-post',[.13,3.8,.13],[b.x,1.9,b.z],steel));}
  }
  this.merge(kerbs,'striped-kerbs');this.merge(walls,'concrete-barriers');this.merge(fences,'fence-posts');
  
for(let row=0;row<2;row++)for(let col=0;col<12;col++){const p=t.at(row*.85,(col-5.5)*1.5),m=this.box('finish-check',[1.5,.025,.85],[p.x,.045,p.z],(row+col)%2?black:white);m.rotation.y=p.heading;}
  ...
  for(let i=0;i<7;i++){this.box('pit-garage',[13,5.5,13],[-60+i*15,2.75,-170],this.mat('garage','#d7dcd8'));this.box('garage-door',[11,3.7,.12],[-60+i*15,1.86,-163.42],this.mat('door','#234557',.3,.5));this.box('glass-office',[12,2.8,10],[-60+i*15,6.9,-170],this.mat('architectural-glass','#396579',.35,.18));this.box('roof',[14,.22,14],[-60+i*15,8.45,-170],white);}
  this.sign('Meridian Motorsport',[-15,10,-167],46,3.5);
  const audience=[];for(let row=0;row<7;row++){this.box('stand-step',[105,.7,2.2],[15,1+row*.7,161+row*2],concrete);for(let seat=0;seat<55;seat++){const m=B.MeshBuilder.CreateSphere('spectator',{diameter:.54,segments:4},this.scene);m.position=v(-35+seat*1.9,1.8+row*.7,161+row*2);m.material=[blue,orange,white,black][(row*7+seat)%4];audience.push(m);}}this.merge(audience,'crowd');
  this.box('stand-roof',[110,.3,19],[15,8,168],white);for(const x of[-38,15,68])this.box('stand-support',[.35,8,.35],[x,4,177],steel);
  
const treeMat=this.mat('leaves','#385646'),trunkMat=this.mat('trunk','#776659'),forest=[];
  for(let i=0;i<120;i++){const a=i*2.39996,r=215+(i%7)*13,x=Math.sin(a)*r,z=Math.cos(a)*r;const trunk=B.MeshBuilder.CreateCylinder('tree-trunk',{height:6,diameter:.55,tessellation:6},this.scene);trunk.position=v(x,3,z);trunk.material=trunkMat;forest.push(trunk);const crown=B.MeshBuilder.CreateCylinder('tree-crown',{height:9,diameterTop:.4,diameterBottom:5+(i%3),tessellation:7},this.scene);crown.position=v(x,8,z);crown.material=treeMat;forest.push(crown);}this.merge(forest,'forest');
  const mountainMat=this.mat('hills','#6b8587');for(let i=0;i<18;i++){const a=i/18*Math.PI*2,m=B.MeshBuilder.CreateCylinder('distant-hill',{height:65+i%4*22,diameterTop:5,diameterBottom:160,tessellation:7},this.scene);m.position=v(Math.sin(a)*650,10,Math.cos(a)*650);m.material=mountainMat;}
  const boardMat=this.mat('board','#f4f4e8');for(let s=100;s<t.length;s+=135){const p=t.at(s,22),node=new B.TransformNode('braking-marker',this.scene);node.position=v(p.x,0,p.z);node.rotation.y=p.heading;this.box('marker',[1.8,1.5,.15],[0,1.4,0],boardMat,node);this.sign('100',[0,1.4,-.09],1.7,.85,node);}
  this.freezeStaticWorld();this.createRain();
 }
...
return{root,body,wheels};
 ...
 setWeather(weather){const wet=weather==='wet';this.scene.clearColor=wet?new B.Color4(.34,.42,.48,1):new B.Color4(.69,.81,.87,1);this.scene.fogColor=wet?color('#71838e'):color('#b7cdd6');this.scene.fogDensity=wet?.0038:.0018;this.sun.intensity=wet?.6:2.1;this.hemi.intensity=wet?.8:.85;this.roadMaterial.roughness=wet?.21:.91;this.roadMaterial.metallic=wet?.28:0;if(this.rain)this.rain.setEnabled(wet);}
