...
test('renders the circuit, starts, accelerates, pauses, repairs, and restarts',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/');await expect(page.locator('#start')).toBeEnabled();await expect(page.locator('#error')).toBeHidden();await page.locator('#quality').selectOption('medium');await page.screenshot({path:'test-results/meridian-menu.png'});
...
test('wet setup, selected car, mute, and quality persist across reload',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/');await expect(page.locator('#start')).toBeEnabled();await page.locator('input[value="titan"]').check();await page.locator('#weather').selectOption('wet');await page.locator('#quality').selectOption('medium');await page.locator('#mute').click();
...
test('storage failure does not prevent gameplay and blur pauses the clock',async({page})=>{
 await page.addInitScript(()=>Object.defineProperty(window,'localStorage',{get(){throw new Error('blocked');}}));await page.goto('/');await expect(page.locator('#start')).toBeEnabled();await expect(page.locator('#storage-status')).toContainText('unavailable');await page.locator('#quality').selectOption('medium');
