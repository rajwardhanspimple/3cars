...
async function modelStats(page){return page.evaluate(()=>{
 ...
 return{triangles:meshes.reduce((sum,m)=>sum+m.getTotalIndices()/3,0),parts:meshes.length,lod:meshes.reduce((n,m)=>n+(m.getLODLevels?.().length||0),0),meta:root?.metadata,min,max,frame:scene.getFrameId(),reflection:!!scene.environmentTexture};
 });}
...
test('Mustang retains every triangle and real wheel geometry in both settings',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('#start')).toBeEnabled({timeout:120000});await expect(page.locator('#model-status')).toHaveAttribute('data-triangles',String(COUNT));
 let stats=await modelStats(page);console.log('Full detail Mustang:',JSON.stringify(stats));expect(stats.triangles).toBe(COUNT);expect(stats.lod).toBe(0);expect(stats.meta.wheelCount).toBe(4);expect(stats.meta.geometrySimplified).toBe(false);expect(stats.reflection).toBe(true);expect(stats.max[1]-stats.min[1]).toBeGreaterThan(1);expect(stats.max[1]-stats.min[1]).toBeLessThan(2);expect(stats.min[1]).toBeGreaterThan(-.15);
 await page.screenshot({path:'test-results/mustang-full-detail.png'});
 await page.locator('#quality').selectOption('medium');await expect(page.locator('body')).toHaveAttribute('data-loading','false',{timeout:120000});stats=await modelStats(page);expect(stats.triangles).toBe(COUNT);expect(stats.lod).toBe(0);
 ...
 stats=await modelStats(page);expect(stats.triangles).toBe(COUNT);await page.screenshot({path:'test-results/mustang-racing-full-geometry.png'});expect(errors).toEqual([]);expect(external).toEqual([]);
});
